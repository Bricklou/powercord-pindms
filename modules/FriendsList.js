...
module.exports = async function () {
	...
	const _injectFriendRow = async () => {
		...
		inject('pd-friendslist-row', FriendRow, 'render', (_, res) => {
			...
			res.props.children = (...args) => {
				...
				children.props.children.splice(1, 0, React.createElement('div', { className: 'pd-mutualGuilds pd-container', onClick: console.log }));
	...
	const _injectPeopleList = async () => {
		...
		inject('pd-friendslist', PeopleListSectionedNonLazy, 'default', (_, res) => {
				...
				children.props.children[0].props.children = children.props.children[0].props.children.map(section => {
					...
					if (this.sortKey) {
						section = section.map(user => {
							...
							user.isPinned = Object.values(this.settings.get('dmCategories')).some(cat => cat.dms.includes(user.key));
							return user;
						});
						if (this.sortKey === 'isPinned') section = section.filter(u => u[this.sortKey]);
						section.sort((x, y) => {
